...
export class AccountPolicyGateContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this.updateManagedSettingsCompatibilityNotification(this.defaultAccountService.managedSettingsCompatibilityError);
		...
		this._register(this.defaultAccountService.onDidChangeManagedSettingsCompatibilityError(error => this.updateManagedSettingsCompatibilityNotification(error)));
	...
	private updateManagedSettingsCompatibilityNotification(error: IManagedSettingsCompatibilityError | null): void {
		if (!error) {
