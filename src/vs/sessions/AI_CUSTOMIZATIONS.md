...
**The switch has exactly one meaning: on or off, durably.** It writes the profile layer, and because writing a profile state also clears any workspace entry, a narrower choice cannot survive to mask what the user just asked for. It aligns the session flag at the same time, so a server turned on can never remain visibly off. An earlier version rewrote whichever layer happened to hold the current choice and named that layer in the label -- which was truthful but meant two identical-looking switches did materially different things, since "off until you turn it back on" and "off until this session ends" are different outcomes, not one action with different metadata. Scope changes remain available in the context menu, where they are spelled out. Servers that exist only inside an agent session are the one exception, and only because the session is the sole layer they have; the control still means "turn this off".

Scope is shown as *state*, never as control semantics. A row held off by a workspace or session choice reads `Off (Workspace)` or `Off (Session)` instead of a bare `Off`, which explains a server the user did not turn off here. The switch's label stays uniform ("Turn off Redis"), because where a server's choice lives is a property of the server, not of the control. Previously the list showed only `Off`, so which layer had disabled a server was discoverable only by opening a menu.
