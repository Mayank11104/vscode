...
The list has exactly two groups, because it previously mixed two axes at once: *where the config lives* (Workspace/User) and *who provided the server* (Plugin/Extension/Built-in), which left no single question a user could answer to predict a server's group. The one axis that matters is who owns the configuration. **Your Servers** holds servers the user configured, which they can edit and remove. **From Installed Software** holds servers that arrived with extensions, plugins, or VS Code, which they can turn off but not edit. Nothing is lost by collapsing, because every row's meta line already names its exact origin.

Clicking a row does one thing per row shape. Group headers collapse. Gallery rows open their detail page, which is the only place a detail page has content. Installed rows do nothing on click: previously a click was a coin flip between opening a panel, opening a quick pick, or doing nothing at all, decided by whether a description happened to exist. Everything an installed row can do now lives in its switch and its context menu.

Every installed row always renders a status, because a row that renders nothing is indistinguishable from a broken one. Status is a short word (`Running`, `Starting`, `Idle`, `Off`, `Failed`, `Sign-in needed`) preceded by a colored dot, placed immediately after the server name so state reads as part of the server's identity. Colour is never the only signal. A server that has no runtime state yet is `Idle`, not blank; only gallery entries the user has not installed have no status. Disabled rows dim the name and description but keep status and error text at full contrast.
...
Every installed row carries an on/off switch at its trailing edge, in the same position on every row, so turning a server off is one predictable target rather than a right-click users have to discover. The switch writes to the scope that row already lives in: session-backed rows toggle the session, locally configured rows toggle the durable enablement and align the session flag with it, so a server turned on can never remain visibly off. Gallery rows have no switch, because there is nothing to turn on until the server is installed. The context menu keeps every scope-changing action, and it stays the only way to move a choice between scopes.

Scope is shown rather than implied. A row held off by a workspace or session choice reads `Off (Workspace)` or `Off (Session)` instead of a bare `Off`, and the switch's hover and accessible name state the reach of the click ("Turn off Redis in this workspace"). Previously the list showed only `Off`, so which layer had disabled a server was discoverable only by opening a menu.
