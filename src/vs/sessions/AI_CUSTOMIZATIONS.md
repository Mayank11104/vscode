...
Two states are deliberately silent. `Running` and `Idle` are lifecycle, not news: MCP servers start lazily, so whether a process is alive at this instant flickers and is not something anyone acts on. A bare `Off` is silent too, because every installed row carries a switch and an off switch already says it -- but `Off (Workspace)` and `Off (Session)` are shown, since the scope is exactly what the switch cannot express. What remains is the set worth interrupting a scan for: `Failed`, `Sign-in needed`, `Starting`, and scope-qualified `Off`.

The accessible name is deliberately looser: it includes a plain `Off`, because a screen reader reads the row label without the switch beside it, so on-screen duplication is new information there. `Running` and `Idle` are omitted from both.
